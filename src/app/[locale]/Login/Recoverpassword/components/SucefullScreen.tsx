...
function SucefullScreen({ email, password}: {email: string, password:string }) {

  const {signIn} = useContext(AuthContext)

  function HandleAcces(email:string, password:string){
  ...
  return (
    <section className="h-[calc(100vh-10vh)] flex flex-col">
      ...
      <div className="h-1/2  flex flex-col justify-end">
        <button type="submit" className="Btn_Primary" onClick={()=>HandleAcces(email, password)}>
          Acessar minha conta
        </button>
      </div>
    </section>
